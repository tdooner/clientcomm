...
class CaptureBoard {
  ...
  static findByOrg(orgId) {
    return new Promise((fulfill, reject) => {
      ...
      .then((floaters) => {
        ...
        let convos = floaters.map(ea => ea.convo).reduce((a, b) => {
          if (a.indexOf(b) < 0) {
            a.push(b);
          }
          return a;
  ...
  static findByConversationId(orgId, conversationId) {
    return new Promise((fulfill, reject) => {
      ...
      .then((conversations) => {
        conversations = conversations.filter(convo => Number(conversationId) == convo.convo);
      ...
      .then((resp) => {
        ...
        // Query 1: Update convo with CM and client
         db('convos')
          ...
          .returning('*')).then(resp => Conversations.findById(conversationId)).then(resp =>
        ...
        // Filter so you only have new comms that CM does not have captured yet
            comms = comms.filter(comm => clientcomms.indexOf(comm) == -1);
