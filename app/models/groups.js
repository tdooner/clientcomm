...
class Groups {
  ...
  static editOne(userID, groupID, name, clientIDs) {
    return new Promise((fulfill, reject) => {
      db('groups')
        .update({ name })
        .where('group_id', groupID)
      .then(() => Groups.updateMembersOne(userID, groupID, clientIDs)).then(() => {
        fulfill();
      }).catch(reject);
    });
  }
  ...
  static updateMembersOne(userID, groupID, clientIDs) {
    return new Promise((fulfill, reject) => {
      ...
      .then((clients) => {
        ...
        clientIDs = clientIDs.filter(ID => activeClients.indexOf(Number(ID)) < 0);
  ...
  static addGroupMembers(userID, groupID, clientIDs) {
    return new Promise((fulfill, reject) => {
      const insertArray = clientIDs.map(clientID => ({
        group: groupID,
        client: clientID,
        active: true,
      }));
